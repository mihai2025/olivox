...
export default function TrackingPixels() {
  ...
  useEffect(() => {
    fetch("/api/pixels")
      .then((r) => r.json())
      .then((data) => { setPixels(data); setLoaded(true); })
  ...
  return (
    <>
      ...
      {pixels.google_analytics && (
        <>
          <script async src={`https://www.googletagmanager.com/gtag/js?id=${pixels.google_analytics}`} />
          <script dangerouslySetInnerHTML={{ __html: `
            window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}
            gtag('js',new Date());gtag('config','${pixels.google_analytics}');
            ${pixels.google_ads_id ? `gtag('config','${pixels.google_ads_id}');` : ""}
          `}} />
        </>
      )}
      ...
      {pixels.pinterest && (
        <script dangerouslySetInnerHTML={{ __html: `
          ...
          pintrk('load','${pixels.pinterest}');pintrk('page');
        `}} />
      )}
    </>
  );
